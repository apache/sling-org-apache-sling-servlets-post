...
public class JavaxToJakartaPostOperation implements JakartaPostOperation {
    ...
    public JavaxToJakartaPostOperation(final PostOperation delegate) {
    ...
    throws PreconditionViolatedPersistenceException, TemporaryPersistenceException, PersistenceException {
        final SlingPostProcessor[] wrappers = new SlingPostProcessor[processors.length];
        for(int i=0; i<processors.length; i++) {
            wrappers[i] = new JakartaToJavaxPostProcessor(processors[i]);
        }
        this.delegate.run(JakartaToJavaxRequestWrapper.toJavaxRequest(request),
            new JakartaToJavaxPostResponse(response), wrappers);
